...
@section('title', __('users.edit_user') . ' - ' . __('users.users'))
    ...
    <div class="col-xl">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('users.edit_user') }}</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('users.update', $user->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            ...
                            <input type="password" class="form-control" id="password" name="password" placeholder="{{ __('users.leave_empty_for_no_change') }}">
                            @error('password')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <select class="form-select select2" id="status" name="status" required>
                            <option value="1" {{ old('status', $user->status) == 1 ? 'selected' : '' }}>{{ __('users.active') }}</option>
                            <option value="0" {{ old('status', $user->status) == 0 ? 'selected' : '' }}>{{ __('users.inactive') }}</option>
                        </select>
                        @error('status')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-success me-2">{{ __('users.update') }}</button>
