...
@section('title', __('Iniciar Montagem'))
    ...
    <div class="col-xl-8 col-lg-8 col-md-10 mx-auto">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Iniciar Montagem') }}</h5>
                <a href="{{ route('assembly-schedules.showDetails', $assemblySchedule->id) }}" class="btn btn-outline-secondary btn-sm">{{ __('Voltar') }}</a>
            </div>
            <div class="card-body">
                <p class="mb-3">{{ __('Para iniciar a montagem, tire uma foto no ambiente onde será realizada. As coordenadas geográficas serão capturadas para confirmar que você está no local da montagem.') }}</p>

                <form id="startAssemblyForm" action="{{ route('assembler.my-schedule.start') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3">
                        <label class="form-label">{{ __('Foto do local da montagem') }}</label>
                        <input type="file" name="start_photo" accept="image/*" capture="environment" class="form-control" required>
                    </div>

                    <div class="mb-3" id="locationStatus" class="text-muted"></div>

                    <button type="submit" class="btn btn-primary">{{ __('Iniciar Montagem') }}</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        requestLocation();
        document.getElementById('startAssemblyForm').addEventListener('submit', function(e) {
            var lat = document.getElementById('start_latitude').value;
            var lng = document.getElementById('start_longitude').value;
            if (!lat || !lng) {
                e.preventDefault();
                setStatus('Localização obrigatória para iniciar a montagem.', 'danger');
            }
        });
    });
</script>
