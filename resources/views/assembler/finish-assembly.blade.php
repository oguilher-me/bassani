...
@section('title', __('Concluir Montagem'))
    ...
    <div class="col-xl-8 col-lg-8 col-md-10 mx-auto">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Concluir Montagem') }}</h5>
                <a href="{{ route('assembly-schedules.showDetails', $assemblySchedule->id) }}" class="btn btn-outline-secondary btn-sm">{{ __('Voltar') }}</a>
            </div>
            <div class="card-body">
                <p class="mb-3">{{ __('Anexe uma ou mais fotos para documentar a montagem e adicione uma observação se necessário.') }}</p>

                <form id="finishAssemblyForm" action="{{ route('assembler.my-schedule.finish') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="assembly_schedule_id" value="{{ $assemblySchedule->id }}">

                    <div class="mb-3">
                        <label class="form-label">{{ __('Fotos da montagem (uma ou mais)') }}</label>
                    ...
                    <button type="submit" id="btnFull" class="btn btn-primary">{{ __('Concluir Montagem') }}</button>
