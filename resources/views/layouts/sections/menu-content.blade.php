...
// Determine menu based on user role at render time
$user = Auth::user();
if ($user && $user->hasRole('Montador')) {
    $assemblerMenuJson = file_get_contents(base_path('resources/menu/assemblerMenu.json'));
    $menuData = json_decode($assemblerMenuJson);
} else {
    $menuData = $menuData ?? json_decode(file_get_contents(base_path('resources/menu/verticalMenu.json')));
}
@endphp
...
<div class="app-brand demo">
    ...
    <a href="javascript:void(0);" class="layout-menu-toggle menu-link text-large ms-auto d-xl-none" id="mobile-close-btn">
